			...
			duplicate_remove: function(options) {
				...
				return this.each(
					function() {
						...
						$basic	= $$.clone();
						$parent	= $$.parent();
						total	= $parent.find($basic.get(0).tagName).length;
						
						$duplicate	= $('<a />').attr('href', o['duplicate']['href']).text(o['duplicate']['text']).addClass('duplicate').addClass('phark');
						$remove		= $('<a />').attr('href', o['remove']['href']).text(o['remove']['text']).addClass('remove').addClass('phark');
