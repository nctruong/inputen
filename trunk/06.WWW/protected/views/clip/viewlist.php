<div id='sub-nav' class='row-fluid'>
    <div class='page3'>
        <div class='title_ab span3'>
            <span class="row-fluid span12 title_small">THỂ LOẠI</span>
            <span class="row-fluid span12 title_big">HỌC QUA CLIP</span>           
        </div>
        <div id='' class='span9 en-title-pg2'>
            <?php
                $total = count($listcat);
                ...
                foreach($listcat as $k=>$v){
                    ...
                    echo "<a  href='".Yii::app()->getBaseUrl(true)."/hoc-qua-clip/".$v->slug."-".$v->id.".html'>".$v->title."</a>";
                    if($i < $total){
                        echo "/";
                    }
                }
            ...
            <?php
                ...
                foreach($items as $k=>$v){
                    ...
                    <div class="row-fluid en-box-title">
                       <a class='title-listen' href="<?php echo Yii::app()->getBaseUrl(true)."/hoc-qua-clip/".$root->slug."-".$root->id."/".$v->slug."-".$v->id.".html"?>" title="<?php echo $v->title ?>"><?php echo $v->title ?></a>
                    </div>
                    <div class="row-fluid en-box-feature "><!-- .en-box-feature -->
                        <div class="row-fluid">
                            <div class="span8 offset2" >
                                <a href="<?php echo Yii::app()->getBaseUrl(true)."/hoc-qua-clip/".$root->slug."-".$root->id."/".$v->slug."-".$v->id.".html"?>" class="thumbnail" title="<?php echo $v->title ?>" ><img class="" src="<?php echo Libraries::getImage($v->content) ?>" /></a>
                            </div>
                        </div>
