        ...
        <div id='' class='span8 en-title-pg2'>
            <?php
                ...
                foreach($listcat as $k=>$v){
                        ...
                        echo "<a  href='".Yii::app()->getBaseUrl(true)."/bai-test/".$v->slug."-".$v->id.".html'>".$v->title."</a>";
                        if($i < $total){
                            echo "/";
                        }
                }  
            ...
            <?php
                ...
                foreach($items as $k=>$v){
                    ...
                    <div class="row-fluid en-box-title">
                       <a class='title-listen' href="<?php echo Yii::app()->getBaseUrl(true)."/bai-test/".$root->slug."-".$root->id."/".$v->slug."-".$v->id.".html"?>" title="<?php echo $v->title ?>"><?php echo $v->title ?></a>
                    </div>
                    <div class="row-fluid en-box-feature "><!-- .en-box-feature -->
                        <div class="row-fluid">
                            <div class="span8 offset2" >
                                <a href="<?php echo Yii::app()->getBaseUrl(true)."/bai-test/".$root->slug."-".$root->id."/".$v->slug."-".$v->id.".html"?>" class="thumbnail" title="<?php echo $v->title ?>" ><img class="" src="<?php echo Libraries::getImage($v->content) ?>" /></a>
                            </div>
                        </div>
